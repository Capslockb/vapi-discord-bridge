...
- The post-call summary helper is owned by the installed plugin at `plugin/post_call_summary.py`, so copy, symlink, and custom/local installs no longer depend on a retained source checkout. `scripts/post_call_summary.py` remains a repository compatibility launcher. This packaging correction does not create transcripts or change transcript authorization boundaries tracked in Issues [#2](https://github.com/Capslockb/vapi-discord-bridge/issues/2) and [#9](https://github.com/Capslockb/vapi-discord-bridge/issues/9).
